...
${errorInfo.componentStack}`);
  ...
  function Extension() {
    const [coupon, getCoupon] = (0, import_react15.useState)("");
    ...
    const applyDiscount = () => __async(this, null, function* () {
      try {
        const response = yield fetch("https://farzipromoshopify.farziengineer.co/discount", {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            "code": coupon,
            "client": "MINIMALISTINC"
          })
        });
        const result = yield response.json();
        console.log(result);
        console.log(shop);
        applyDiscountCodeChange(coupon);
      } catch (err) {
        console.log(err);
      }
    });
